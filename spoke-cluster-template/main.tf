...
module "eks" {
  ...
  eks_managed_node_groups = try(var.enable_existing_eks_managed_node_groups, false) ? try(var.existing_eks_managed_node_groups,
  null) : {
    initial = {
...
module "vpc" {
  count = try(var.create_vpc, false) ? 1 : 0
